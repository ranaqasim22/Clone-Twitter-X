...
function TweetCard({ tweet }) {
  ...
  return (
    <article className="relative flex gap-3 border-b border-twitter-lightGray px-4 py-3 transition hover:bg-white/[0.03]">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex min-w-0 items-center gap-1 pr-8 text-sm">
          <Link to={`/profile/${tweet.user.username}`} className="truncate font-bold text-white hover:underline">{tweet.user.name}</Link>
        ...
        <p className="whitespace-pre-wrap break-words text-[15px] leading-5 text-white">{tweet.content}</p>

        {tweet.quoteTweet && <div className="mt-3 rounded-2xl border border-twitter-lightGray p-3"><div className="flex items-center gap-2"><img src={tweet.quoteTweet.user.avatar} alt="" className="h-5 w-5 rounded-full" /><span className="text-sm font-bold text-white">{tweet.quoteTweet.user.name}</span><span className="text-sm text-twitter-gray">@{tweet.quoteTweet.user.username}</span></div><p className="mt-2 whitespace-pre-wrap text-sm text-white">{tweet.quoteTweet.content}</p>{tweet.quoteTweet.media?.[0] && <img src={tweet.quoteTweet.media[0]} alt="Quoted post attachment" className="mt-3 h-40 w-full rounded-xl object-cover" />}</div>}
        ...
        <div className="absolute right-2 top-2">
          ...
          {menuOpen && (
            <div className="absolute right-0 z-10 mt-1 w-32 rounded-xl bg-twitter-dark py-1 shadow-lg ring-1 ring-twitter-lightGray">
              <button type="button" onClick={() => dispatch(deleteTweet(tweet.id))} className="flex w-full items-center gap-2 px-3 py-2 text-sm text-red-500 hover:bg-white/5"><Trash2 className="h-4 w-4" />Delete</button>
            </div>
          )}
        </div>
      )}
