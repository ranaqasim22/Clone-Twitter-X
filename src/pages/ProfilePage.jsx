    ...
    <>
      ...
      <section>
        ...
        <div className="relative px-4 pb-4">
          ...
          {isCurrentUser ? <button type="button" onClick={() => navigate('/settings/profile')} className="absolute right-4 top-3 rounded-full border border-twitter-lightGray px-4 py-2 font-bold text-white hover:bg-white/10">Edit profile</button> : <button type="button" onClick={() => dispatch(toggleFollowing(profileId))} className={`absolute right-4 top-3 rounded-full px-4 py-2 font-bold ${isFollowing ? 'border border-twitter-lightGray text-white hover:bg-red-500/10 hover:text-red-400' : 'bg-white text-twitter-darker hover:bg-twitter-lightGray'}`}>{isFollowing ? 'Following' : 'Follow'}</button>}
          <h2 className="mt-3 text-xl font-extrabold text-white">{profile.name}</h2>
          <p className="text-twitter-gray">@{profile.username}</p>
          {profile.bio && <p className="mt-3 whitespace-pre-wrap text-white">{profile.bio}</p>}
          ...
          <div className="mt-3 flex gap-4 text-sm"><span className="text-twitter-gray"><strong className="text-white">{profile.following || 0}</strong> Following</span><span className="text-twitter-gray"><strong className="text-white">{profile.followers || 0}</strong> Followers</span></div>
        </div>
      </section>
      <div className="flex border-b border-twitter-lightGray">
        {tabs.map((tab) => <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`relative flex-1 py-4 text-sm ${activeTab === tab ? 'font-bold text-white' : 'text-twitter-gray'}`}>{tab}{activeTab === tab && <span className="absolute bottom-0 left-1/2 h-1 w-10 -translate-x-1/2 rounded-full bg-twitter-blue" />}</button>)}
      </div>
